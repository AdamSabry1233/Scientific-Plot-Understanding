...
OUT_IMG = Path("synthetic_output/images")
OUT_LBL = Path("synthetic_output/labels")
OUT_JSON = Path("synthetic_output/ground_truth")
OUT_MASK = Path("synthetic_output/curve_masks")
...
def generate_plot(i, seed=None):
    ...
    # Labels: sometimes blank (80% chance to have label, 20% blank)
    x_label = ""
    if random.random() < 0.8:
        x_label = random.choice(X_LABELS)
    
    y_label = ""
    if random.random() < 0.8:
        y_label = random.choice(Y_LABELS)
    
    ax.set_xlabel(x_label)
    ax.set_ylabel(y_label)

    # Title: sometimes blank (75% chance to have title, 25% blank)
    title = ""
